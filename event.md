...
A dot `.` is used to separate the 3 parts that define the action field (serviceName, objectType, action).

A dash `–` is used to separate complex objectTypes to make it more readable to the users.
...
This field provides information about username of the initiator that initiated the action.
{: note}
...
When the action is not a REST API call, set to `200` for success outcome and `500` for failure.
...
This field is optional. When the field is available, it includes additional information about the request.  
{: note}
...
This field is optional and defines the IP Address or URL of the target service.
{: note}
