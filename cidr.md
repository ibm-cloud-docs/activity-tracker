...
Classless Inter-Domain Routing (CIDR) blocks are ranges of addresses. {{site.data.keyword.at_full}} traffic will be sent from these addresses. This traffic includes webhooks, archiving to {{site.data.keyword.cos_full_notm}}, streaming to {{site.data.keyword.messagehub}} and so on. You can use these CIDR blocks in firewalls and other configurations to ensure that the data received by your code, or another {{site.data.keyword.IBM_notm}} service, originates only from {{site.data.keyword.at_full_notm}}.
{: shortdesc}
