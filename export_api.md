...
| `query`     | `string`     | Optional   | [Search query](/docs/activity-tracker?topic=activity-tracker-views#views_step2).  </br>You can test your query in a view in the UI. Then, when you copy the query to use for the export, replace each blank space in the query with `%20`.|
...
3. Define the search query. 
    ...
    To get more information on how to search, see [Select the set of events to display through a view by applying a search query](/docs/activity-tracker?topic=activity-tracker-views#views_step2).
