...
The steps in this section assume that you have a list of all {{site.data.keyword.at_full_notm}} instances for each resource group and region in the account; access reports for each instance; and a current export of the configuration resources for each instance.
{: important}
...
4. For each instance, you must import the auditing configuration so any views, parsing templates, boards and screens are uploaded into the new instance. For more information, see [Import the configuration of resources into a auditing instance](/docs/activity-tracker?topic=activity-tracker-reuse_resource_definitions#import_config).

5. If the region that goes down is Frankfurt, an additional step is required. Global events are sent to the {{site.data.keyword.at_short}} instance in Frankfurt. In the event of a DR for the Frankfurt region, you must configure global events through {{site.data.keyword.atracker_short}} to be routed to the auditing instance that you have provisioned in the new region.

    To learn how to configure global events to be routed to the new instance, see [Configuring {{site.data.keyword.atracker_short}} in your account](/docs/atracker?topic=atracker-configure-atracker).



If you recover IBM Cloud services across different regions, these services will generate location-based events in the region where they are operational. To get a similar audit trail as in the region that went down, consider streaming events to 1 auditing instance so that you can group events for analysis. For more information, see [Configuring streaming to an {{site.data.keyword.at_short}} instance](/docs/activity-tracker?topic=activity-tracker-streaming-configure-l2l).
{: note}
...
A multizone region (MZR) consist of 3 or more availability zones that are independent from each other to ensure that single failure events affect only a single zone. By default, {{site.data.keyword.at_short}} hosted event search is deployed across 3 zones, one primary zone and two secondary zones:
...
### Data availability for {{site.data.keyword.at_short}} hosted event search
...
{{site.data.keyword.at_full_notm}} hosted event search follows {{site.data.keyword.cloud_notm}} requirements for [planning and recovering from disaster events](/docs/overview?topic=overview-zero-downtime#disaster-recovery).
{: note}
